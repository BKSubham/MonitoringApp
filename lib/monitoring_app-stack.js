...
class MonitoringAppStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    const dbLambdaFunction = new lambda.Function(this, 'NotificationHandler', {
      runtime: lambda.Runtime.NODEJS_16_X,
      code: lambda.Code.fromAsset('dbLambda'),
    ...
    table.grantReadWriteData(dbLambdaFunction);
  ...
  const alarmTopic = new sns.Topic(this, 'AlarmTopic', {
    ...
    const { emails } = JSON.parse(rawEmailsData);
     ...
     alarmTopic.addSubscription(new sns_subscriptions.LambdaSubscription(dbLambdaFunction));
