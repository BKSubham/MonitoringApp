...
class MonitoringAppStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    websites.forEach((website) => {
      ...
      // Define the SNS Topic for alarms
      const alarmTopic = new sns.Topic(this, 'AlarmTopic', {
        displayName: 'Alarm Notifications'
      });
    // Add email subscription to the SNS topic
    alarmTopic.addSubscription(new subs.EmailSubscription('dhunganaprajwol237@gmail.com'));
        // Add SNS action to the alarm
        alarm.addAlarmAction(new actions.SnsAction(alarmTopic, {
          messageBody: `Alert: ${metricName} for ${website} has triggered an alarm. Please check the website.`,
        }));
      });
    });
  }
